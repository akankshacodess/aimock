...
export default function Feedback({params}) {
    ...
    const averageRating = feedbackList.length > 0 ? (totalRating / feedbackList.length).toFixed(1) : "N/A";
        ...
        {feedbackList&&feedbackList.map((item, index)=>(
          <Collapsible key = {index} className='mt-7'>
            ...
            <CollapsibleContent>
              <div>
              <h2>
                <strong className='text-red-500 p-2 rounded-lg'>Rating: </strong>
                {item.rating ?? 'N/A'}
              </h2>
                <h2 className='p-2 border rounded-lg bg-red-500 text-sm  text-red-100'><strong>Your Answer </strong>{item.userAns}</h2>
                <h2 className='p-2 border rounded-lg bg-green-500 text-sm  text-green-100'><strong>Correct Answer </strong>{item.correctAns}</h2>
                <h2 className='p-2 border rounded-lg bg-blue-500 text-sm  text-red-100'><strong>Feedback: </strong>{item.feedback}</h2>
